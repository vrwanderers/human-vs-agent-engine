...
| [Character is Destiny / LIFECHOICE](https://aclanthology.org/2025.findings-emnlp.813/) | 角色理解应落实为根据前文与人物状态预测下一项关键抉择，而不只是模仿口吻 | 离线结构化校准案例、通用动机冲突模型、冻结标签防泄漏测试 | 决策匹配、动机排序、歧义—置信度校准 |
...
- 文学中的越界抉择暴露出“价值一致性过强”也会显得不真实。MVP-6 用冲动压力、社会易感性和自我许可表示诱惑、施压与合理化；MVP-7 再记录价值、关系与承诺债务，并让高风险选择在后续回合成熟。对应机制必须产生可观察事件和消融差异，避免只增加无法证伪的心理名词。

## 评价和负面对照

MVP-8 保留六维总分，在 `ai_human_likeness` 中加入：
...
11. 冲动、社会压力和自我许可是否以渐变而非固定模板影响行动。
12. 价值、关系与承诺冲突是否形成可修复但不会自动清零的债务。
13. 高风险选择是否在后续回合产生可观察的延迟后果，而不是停留在提示词描述。

自动化负面对照至少包括平坦重复角色、无差别随机波动、无证据反思、每回合重置计划、
永远选择效用最大值，以及不受情境影响的固定人格。最终发布门槛必须再加入真人双盲 A/B：
将 MVP-3、MVP-4、MVP-5、MVP-6、MVP-7、MVP-8、真人脚本和随机/平坦对照混排，评价角色一致、自然程度、情绪因果、
社会理解、决策可信、故事吸引力和是否愿意继续互动；不能只问“你觉得它是不是人”。

著名文学、影视与传记人物只构成另一条 `character_reference_fidelity` 证据链。内置 38 例原型集使用原创转述、无剧本/字幕/对白或传记段落，并把虚构人物与传记人物分轨；传记叙述仍不是行为遥测。其高分只能说明当前机制能重现这些设计案例，不能替代真实玩家实验。离线校准案例不会成为运行时动作池；独立人物卡只提供身份先验。数据、版权门禁、负面对照、跨作品留出和校准状态见 [人物决策参考校准](NARRATIVE_CHARACTER_CALIBRATION.md) 与 [人物卡](CHARACTER_CARDS.md)。
