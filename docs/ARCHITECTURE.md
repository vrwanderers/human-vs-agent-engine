...
Agent 认知循环升级为 `观察 → 评价/应对 → 情境人格激活 → 显著性记忆检索 → 社会信念 → 持续计划 → 快/慢有限理性决策 → 内在状态/公开表达分离 → 合法性硬校验 → 结果记忆与证据反思`。心理矩阵包括信心、士气、压力、沮丧、愤怒、恐惧、疲劳、不确定性和社会信任；这些状态直接进入动作效用，而不是只用于展示。社交 MOD 可以把多个合法宏观策略按连续权重混合，规则层校验主要策略和所有混合分量后再计算后果。
