...
本套件把著名文学、戏剧、电影/剧本叙事、电视剧和机构传记中的关键人物抉择转换为结构化“决策卡”，用于检验引擎是否能从动机冲突、承诺、秘密压力、人格倾向、处境评价和后果遗留推导出连贯选择。虚构人物测量的是**对人类创作叙事人物的拟合度**；传记人物测量的是**对选择性历史叙述的拟合度**。两者都不是现实人群实验或行为遥测，也不能证明 Agent 已达到真人水平。
...
内置 v1 基础集与 v2 补充集共含 25 张原创结构化决策卡，覆盖小说、戏剧、电影、电视剧和机构传记。v2 新增《麦克白》《玩偶之家》《十二怒汉》《卡萨布兰卡》《教父》《正午》，以及林肯、弗雷德里克·道格拉斯、格兰特、曼德拉、马拉拉和凯瑟琳·约翰逊等传记人物。每张卡只保存作品/人物元数据、短篇原创转述、人物状态、候选选择和观察到的选择；不保存原文、剧本、字幕、对白、传记段落或长摘录。
...
预测器不读取 `observed_option`，测试会替换真值标签并验证预测不变。套件还与“永远选第一项”“只顾自保”“只守承诺”三个负面对照比较，并报告判别余量。MVP-7 另外按作品分组进行宏平均和互斥 holdout，按虚构/传记、媒介、决策领域和证据等级分别报告；同一作品不得同时进入训练与 holdout。

《麦克白》最初暴露了机制缺口：只使用价值与承诺时，模型错误预测角色保持忠诚。加入诱惑、社会施压和自我合理化后可以重现观察选择；对应消融仍会失败，从而证明新增机制对该案例有可审计贡献。但这些字段仍由作者设计，不能作为独立验证。

## 当前结果与正确解读

当前 25 例原型集的综合分和按作品宏平均均为 `0.963`，决策匹配率 `1.000`，相对最强负面对照的判别余量为 `0.120`。移除诱惑、社会压力和合理化后，准确率降至 `0.960`，并在《麦克白》失败。6 个 holdout 作品与其余作品无交集，但案例、标签和模型字段仍由同一轮作者设计，存在明显设计者泄漏与过拟合风险，不是真正独立盲测。
