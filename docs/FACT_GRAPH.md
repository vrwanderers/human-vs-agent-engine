...
| 生活回忆 | 家庭、子女、地点和日常经历等人物卡注入记忆 | 否 | 私有，按相关性检索 |
| 关系信念 | 对特定玩家的态度、印象和敏感主题摘要 | 是 | 私有，明确不是对方正史 |
...
- 不允许写入 `identity.*`、`history.formative_memory.*` 或 `history.lived_memory.*`。
- 同一关系已有动态事实时，新事实必须显式提供 `supersedes_fact_id`。
- 被替代事实改为 `superseded`，不会删除，便于回放和审计。
- 拒绝的候选也记录原因，便于评价模型的事实约束能力。
- 战略影响真实度低于阈值的回合启用事实防火墙：该轮所有模型事实提案均拒绝。欺骗可以改变游戏内对手的信念，但不能把说辞写成 Agent 的正史。
